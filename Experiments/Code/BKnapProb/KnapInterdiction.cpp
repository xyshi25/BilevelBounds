...
public:
	BKP()
		: n_(0), ubudget_(0), lbudget_(0), cost_(env), uweight_(env), lweight_(env) {}
	BKP(string filename, string file_path) {
	...
	void Solver_CCLW(ofstream &out_result) {
		try
		{
			...
			// cplex solver
			string cplex_log = "Log/cplex_log_" + filename_ + "_CCLW.log";

			IloCplex mip_cplex(mip_model);
			mip_cplex.setParam(IloCplex::TiLim, 3600);
			...
			out_result << opt_w << '\t' << ng_cuts_bound[1] << '\t' << iter << '\t' << (float)(end_time - start_time) / CLOCKS_PER_SEC << '\t';
			mip_cplex.end();
			ll_cplex.end();
		}
	...
	bool Solver_local_ext(int K, ofstream &out_result) {
		...
		try
		{
			...
			string cplex_log = "Log/cplex_log_" + filename_ + "_" + to_string(K) + "_ext.log";
			...
			out_result << lb << '\t' << ub << '\t' << float(end_t - start_t) / CLOCKS_PER_SEC << '\t';
	...
	bool Solver_local(int K, ofstream &out_result) {
		...
		try
		{
			...
			IloRangeArray mixing_set(env);
			for (size_t i = 0; i < rhweight.size(); i++) {
				mixing_set.add(IloScalProd(lweight_, yVars) + rhweight[i] * wVars[i] >= rhweight[i]);
			}
			model.add(mixing_set);
			mixing_set.end();
			...
			cplex.setParam(IloCplex::TiLim, 3600);
			string cplex_log = "Log/cplex_log_" + filename_ + "_" + to_string(K) + ".log";
			ofstream out_file(cplex_log, std::ios_base::out);
			cplex.setOut(out_file);
