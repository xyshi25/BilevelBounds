...
public:
	...
	BVC(string filename, string file_path, bool flag) {
...
int main() {
	...
	*********************************************************************************/
	...
	//string file_name = "BVC_n5.txt";
	/*string file_name = "Dataset/BVC/BVC_n10d5b5j0.txt";
	BVC test_prob(file_name);
	...
	ofstream out_file("BVC_results.txt");
	string file_path = "../../Dataset/BVC/";
	bool flag = 0; // flag = 1 for interdiction setting
	for (int n = 10; n <= 50; n += 5)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			cout << file_name << endl;
			BVC test_prob(file_name, file_path, flag);
