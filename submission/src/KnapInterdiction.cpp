...
public:
	...
	void Solver_CCLW(ofstream &out_result) {
		try
		{
			...
			out_result << opt_w  << '\t' << float(std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count()) / 1000  << '\t';
			mip_cplex.end();
			ll_cplex.end();
		}
	...
	bool Solver_local_ext(int K, ofstream &out_result) {
		...
		try
		{
			...
			for (size_t i = 0; i < rhweight.size(); i++)
			{
				...
				{
					mixing_con += (rhweight[i] - mu_) * wVars[i];
				}
				else
				{
					mixing_con += (rhweight[i] - rhweight[i + 1]) * wVars[i];
					mixing_set.add(wVars[i] - wVars[i + 1] >= 0);
				}
	...
	bool Solver_local(int K, ofstream &out_result) {
		...
		try
		{
			...
			out_result << left_T1.size() + left_T2.size() << '\t' << rhweight.size() << '\t' << lb << '\t' << ub << '\t' << float(std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count()) / 1000 << '\t';
			cout << left_T1.size() + left_T2.size() << '\t' << rhweight.size() << '\t' << lb << '\t' << ub << '\t' << float(std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count()) / 1000 << endl;
...
int main() {
	...
	**********************************************************************************************/
	...
	out_file.open("DNeg_n15r4_results.txt");
	...
	out_file.open("DNeg_results.txt");
